...
ommt.cache.global_context_cache.platform = omm.Platform.getPlatformByName('CUDA')

pdb = omm.app.PDBFile('mol.pdb')
modeller = omm.app.Modeller(pdb.topology, pdb.positions)
forcefield = omm.app.ForceField('mol.xml', '../jm_gmx_tip3p.xml') # tip3p tip4pew tip4pfb
...
if os.path.exists('sol.nc'):
	simulation = multistate.ReplicaExchangeSampler.from_storage('sol.nc')
else:
